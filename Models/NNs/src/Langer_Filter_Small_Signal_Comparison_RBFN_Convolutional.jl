...
using Plots, Optim, BlackBoxOptim, LineSearches, FFTW, DSP, MATLAB, JLD
pyplot()
# @mput
# mat"
...
ttt = load("C:\\Users\\TET1\\Desktop\\my_files\\Harmonic_Inversion\\Langer_Signals_CST\\4_5_GHz\\ttt_sim.jld")["ttt_sim"]
yyy = load("C:\\Users\\TET1\\Desktop\\my_files\\Harmonic_Inversion\\Langer_Signals_CST\\4_5_GHz\\yyy_sim.jld")["yyy_sim"]
...
mat"
cd 'C:\\Users\\TET1\\Desktop\\my_files\\Harmonic_Inversion\\Prony_MatLab'
sigfile = 'signal4_5.txt';
data = readcols(sigfile);
ttt = data(:,1);
yyy = data(:,2);
f1     = 3e9;  % lower frequency (not important, sorting of results only)
f2     = 6e9;  % higher freq., used for lowpass filtering !
N      = 40;    % order of method, should be 10 < N < 100
nstart = 1000;  % first datapoint in input signal after Gaussin excitation

options.lowpass = 0;

FIL = M2M_prony(ttt(1:2000),yyy(1:2000), f1,f2, N, nstart, options);

fp = linspace(FIL.f1,FIL.f2,10000);
Yp = prony_spectrum(FIL,fp);
...
time_domain_fit=plot(ttt_long[1000:end], yyy_long[1000:end],  lw=3.5, label = "Orig", color=:green,markersize = 5, 	xlims = (1.2e-8,2.5e-7), xticks=(round.(1.34e-8:8e-8:2.5e-7, digits=10)))
plot!(time_span_10Ext[500:10:end], sol_fit_NbExt[500:10:end],markersize = 20,lw=10, color=:black,seriestype = :scatter,m = [:+],label="Ext", ylabel="Amp.", xlabel="s",legendfontsize = 28, guidefont = (28), tickfont = (27))
plot!(time_span_10Ext[1:10:500], sol_fit_NbExt[1:10:500],markersize = 12,lw=2.5, color=:black,seriestype = :scatter,m = [:.],label="Fit")
plot!(t_prony[:], y_prony[:],  lw=2.5, label = "Training", color=:red, m = [:+],seriestype = :scatter,markersize = 17)#, xaxis = ((tstart,6*tend), tstart:2.5*10^-7:6*tend))
y1=[-0.08, 0.08]
...
plot!(freqzz_Original_Ext[9165:9387], abs.(F_Original_Ext)[9165:9387], label ="Spec-Orig (T_{fin})", color=:green,  lw=5, line = (:dashdot, 8),uselatex=true)
...
plot!(freqzz[1178:1207], abs.(F_original[1178:1207]), label = "Spec-Orig (T_{max})", color=:cyan,  lw=5, line = (:dashdot, 7))
